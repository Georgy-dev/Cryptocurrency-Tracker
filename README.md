...
- Ядро: React (JSX)
- Сборка: Vite (обеспечивает быструю разработку и сборку)
- Язык: JavaScript (ES6+)
- Стилизация: TailwindCSS + Any Design (для кастомных компонентов)
- HTTP-клиент: Axios (для удобной работы с запросами к бэкенду)
- Хуки: useState, useEffect для управления состоянием и жизненным циклом компонентов.

### Backend

- Ядро: Python 3.11+
- Фреймворк: FastAPI (современный, высокопроизводительный веб-фреймворк)
- Кеширование: async_lru (декоратор для кеширования результатов асинхронных функций)
